...
### 파일 관리
- **업로드** — 버튼 또는 **드래그 앤 드롭**으로 여러 파일 동시 업로드. **폴더째 업로드**(하위 폴더 구조 보존)와 **여러 폴더 한꺼번에** 올리기도 지원. 같은 이름이 있으면 `이름 (1).ext` 형식으로 자동 회피
...
iOS/Android 앱에서 **서버 주소를 직접 입력하지 않고** QR 스캔만으로 로그인합니다. (Android 앱에 구현되어 있습니다.)
...
│   ├── auth.py               #   계정 생성/로그인/세션/비밀번호 변경 + QR 로그인
