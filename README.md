...
- 관리자는 헤더의 **👥 사용자 관리** 버튼으로 사용자 목록(사용량/제한, 접근 저장소 포함)을 보고 관리합니다
...
- **외부 저장소 권한** — 일반 사용자 행의 **저장소 권한** 버튼으로 접근 가능한 마운트를 체크박스로 선택합니다. 부여되지 않은 저장소는 목록에 아예 보이지 않고 API로 직접 접근해도 차단됩니다. 관리자는 항상 모든 마운트에 접근합니다
...
2. 앱이 QR을 스캔 → `ncloud://login?server=<서버주소>&token=<일회용토큰>` 획득 (서버 주소 + 토큰 동시 전달)
...
image: ghcr.io/nerdnam/ncloud-server:0.0.2
```
...
| POST | `/users/mounts` | 접근 가능한 외부 저장소 목록 설정 |
