# ncloud-sync (Windows 클라이언트)

ncloud 서버에 연결하는 Windows용 프로그램. 두 가지 방식으로 씁니다.
...
- **바로 받기**: [GitHub Releases](https://github.com/nerdnam/ncloud-server/releases)에서 `ncloud-sync-<버전>.exe` 다운로드 (파이썬 불필요, 더블클릭 실행). 버전이 올라갈 때마다 자동으로 게시됩니다.
- **소스로 실행** (파이썬 설치 시):
...
1. 서버 주소·아이디·비밀번호를 넣고 **로그인** → 세션 토큰이 저장됩니다 (비밀번호는 저장 안 함).
2. **일반 디스크로 쓰려면**: 드라이브 문자를 고르고 **드라이브 연결**. 탐색기에 네트워크 드라이브로 나타납니다.
3. **폴더 동기화하려면**: 저장소(개인 `home` 또는 접근 권한 있는 외부 저장소)와 로컬 폴더를 지정하고 **자동 동기화 켜기** 체크. 주기(초)마다 동기화합니다.

## 동기화 동작

- **양방향** — 로컬/원격 어느 쪽에서 만들고·수정·삭제해도 반영됩니다.
- **충돌 안전** — 같은 파일을 양쪽에서 서로 다르게 고치면, 로컬본을 `이름 (conflict 시각).확장자` 로 보존하고 원격본을 받아옵니다. 데이터를 잃지 않습니다.
- **상태 추적** — 로컬 폴더의 `.ncsync\state.json` 에 마지막 동기화 상태를 저장해 신규/수정/삭제를 구분합니다.
...
- WebDAV 드라이브 연결은 **HTTPS 서버**를 권장합니다. 평문 HTTP면 Windows WebClient가 기본적으로 Basic 인증을 막습니다 (레지스트리 `BasicAuthLevel` 조정 필요).
